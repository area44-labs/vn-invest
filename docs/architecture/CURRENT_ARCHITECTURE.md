...
| **Risk Engine**             | `scripts/lib/risk.py`                 | T+2.5 Vietnam settlement risk calculation (VaR 95%, ES 95%, 60d volatility, max drawdown, liquidity score percentile normalization).                                                    | `numpy`, `pandas`, `scripts.lib.recommendation`                                                                             | **MEDIUM** (Circular dependency risk: imports `scripts.lib.recommendation` for formatting helper)                       |
...
| **Test Suites**             | `scripts/tests/test_*.py` (20 files)  | Unit, integration, anti-lookahead, unit normalization, schema, parity, and drift monitoring tests.                                                                                      | `unittest`, `pandas`, `numpy`, `jsonschema`                                                                                 | **LOW** (Comprehensive 456 test coverage)                                                                               |
...
1. **Circular Dependency:**
   - `scripts/lib/risk.py` imports `format_vnd` from `scripts.lib.recommendation` (line 6: `from scripts.lib.recommendation import format_vnd`).
   - `scripts/lib/recommendation.py` imports `calculate_t25_risk_metrics` from `scripts.lib.risk` (line 17: `from scripts.lib.risk import calculate_t25_risk_metrics`).
   - _Impact:_ Creates a tight circular coupling between risk metrics and recommendation formatting. `risk.py` should be a pure quantitative module with zero dependence on recommendation presentation logic.
...
2. **Excessive Coupling & Duplicated Configuration Re-Exports:**
   ...
   - `scripts/lib/recommendation.py` re-exports these exact same constants (lines 28–50).
...
3. **In-Place Mutation Side Effects:**
   - `normalize_universe_liquidity_scores` in `scripts/lib/risk.py` takes a list of recommendation dictionaries and mutates `rec['risk_metrics']['liquidity_score']` in-place.
...
1. **Circular Import Dependency Between Quant Modules (`risk.py` <-> `recommendation.py`)**
   - **Evidence:** `scripts/lib/risk.py` imports `from scripts.lib.recommendation import format_vnd`. `scripts/lib/recommendation.py` imports `from scripts.lib.risk import calculate_t25_risk_metrics`.
   - **Why it matters:** Violates layered software architecture. Quantitative risk calculation relies on recommendation formatting helpers.
   - **Affected Modules:** `scripts/lib/risk.py`, `scripts/lib/recommendation.py`
   - **Potential Consequence:** Import errors during refactoring, inability to run risk module independently in isolated test environments.
   - **Recommended Direction:** Move formatting functions (`format_vnd`) to a utility module (e.g., `scripts/lib/formatting.py`) or keep risk purely numeric.
...
2. **Hardcoded Candidate Stock Universe in Source Code (`scripts/lib/vietnam_market.py`)**
   ...
   - **Recommended Direction:** Externalize universe candidate definitions into JSON/YAML configuration files or universe snapshot files.

### Medium Priority Problems

1. **In-Place Mutation of Recommendation Payloads during Liquidity Normalization**
   - **Evidence:** `normalize_universe_liquidity_scores` in `scripts/lib/risk.py` modifies dictionary items in-place.
